...
| --- | --- | --- | --- | --- | --- |
| D01 | `luces-en-avenida-bisquertt` | CSV `Draft=true` | HTML publicado | Podria excluirse por error si se filtra por draft | Incluir en replica base por existir en mirror; registrar inconsistencia |
| D02 | `through-the-mist` | CSV noticia draft | Sin HTML | Podria publicarse contenido demo | No publicar en replica base; conservar en datos crudos |
| D03 | Categorias demo | CSV tiene Nature, Travel, Vacation, etc. | Sin HTML publico | Riesgo de publicar basura demo | No publicar salvo aprobacion |
| D04 | Autores demo | CSV tiene Mat Vogels y William Wong | Sin paginas publicas | Riesgo editorial | No usar como autores visibles en replica |
| D05 | `Sustentabilidad` duplicada | `sustentabilidad` y `sustentabilidad-2` | Ambas tienen HTML | SEO y navegacion duplicada | Mantener ambas en replica; proponer canonica luego |
...
| D08 | Rotulo `Home` | Mirror usa Home | Informe propone Inicio | Cambio visible | Mantener Home en replica base; cambiar solo en FASE 6 si se aprueba |
| D09 | Rotulo `Blog` | Mirror usa Blog | Informe propone Historias | Cambio de arquitectura editorial visible | Mantener Blog en replica base; evaluar en FASE 6 |
...
| D15 | Formularios | Webflow `method=get` sin destino | Necesidad de contacto real | Funcionalidad incompleta | Implementar estado documentado o pedir backend |
...
- `docs/contents/...Authors...csv`: ambos autores.
- `through-the-mist`.
