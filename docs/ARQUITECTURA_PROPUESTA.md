...
8. Generar `content-manifest.json` con contenido esperado, ruta generada, HTML fuente, CSV fuente, imagenes esperadas/resueltas, video, estado de validacion y `canonicalGroup`.
...
```json
{
  ...
  "mirrorPath": "data/descarga-barrio-bisquertt/...",
...
| Formulario | Endpoint configurable; si no hay endpoint, no envia, no muestra exito falso e informa canal no habilitado |
...
- `/patrimonio`
- `/patrimonio/[slug]`
- `/categories/[slug]` para las cinco categorias con HTML publico: `barrio-bisquertt`, `cultura`, `cursos`, `sustentabilidad`, `sustentabilidad-2`.

Compatibilidad:

- Reglas de redireccion para `.html`.
- No crear redirecciones entre duplicados durante replica base; agrupar con `canonicalGroup`.
...
- Las rutas duplicadas publicas permanecen accesibles y se agrupan con `canonicalGroup`.
